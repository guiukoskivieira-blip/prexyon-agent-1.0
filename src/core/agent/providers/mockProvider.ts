...
export function parseResizeCommand(text: string): { width_mm?: number; height_mm?: number; keepAspectRatio: boolean } | null {
  ...
  if (parsed && !parsed.isAmbiguous) {
    return {
      ...(parsed.width_mm !== undefined ? { width_mm: parsed.width_mm } : {}),
      ...(parsed.height_mm !== undefined ? { height_mm: parsed.height_mm } : {}),
      keepAspectRatio: parsed.keepAspectRatio ?? true,
    };
  }
...
export function createDeterministicTurnsForRequest(
  ...
  // 0. Ambiguidade explícita resolvida pelo Planner
  const plan = buildActionPlanFromUserRequest(message, doc, selectedNodeId);
  if (plan.intent === 'ASK_USER') {
    return [
      {
        response: {
          text: plan.ambiguityQuestion || 'Você quer aplicar essa medida na largura ou na altura da imagem?',
          finishReason: 'STOP',
        },
      },
    ];
